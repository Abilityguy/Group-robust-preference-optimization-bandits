...
class GroupRobustDirectPolicyOptimizationVectorised:
    ...
    def batch_update_once_NONVEC(
        self, dataset: List[GroupTransition], batch_size: int, unique_group_ids: Set[int]
    ) -> float:
        ...
        if self.weighted_batches == True:
            if len(unique_group_ids) == self.group_num:
                group_id_mat = np.floor(np.random.uniform(0, self.group_num, size=(batch_size))).astype(
                    int
                )  # sample group_ids according to batchsize
                # print(group_id_mat)
            else:
            ...
            sampled_group_transitions = [
                sample_group_transition(group_id) for group_id in group_id_mat
            ]  # within that group choose a transition
        else:
        ...
        for transition in sampled_group_transitions:
            ...
            if self.ipo_grad_type == "linear":
                ...
                log_diff = np.log(
                    (cur_policy_act_prob[pref_act] * ref_policy_act_prob[non_pref_act])
                    / (cur_policy_act_prob[non_pref_act] * ref_policy_act_prob[pref_act])
                    + 1e-6
                )
            ...
            if self.ipo_grad_type == "linear":
                ...
                group_loss[group_id] += np.square((log_diff - 0.5 * (1 / self.reg_coef))) + self.adj[
                    group_id
                ] / np.sqrt(self.group_counts[group_id])
            elif self.ipo_grad_type == "justdpo":
                group_loss[group_id] = -np.log(sigmoid(log_ratio_diff)) + self.adj[group_id] / np.sqrt(
                    self.group_counts[group_id]
                )  # calculate group losses
            else:
                raise ValueError("value not implemented")
            ...
            if self.importance_sampling == False:
                ...
                self.group_weights[group_id] = self.group_weights[group_id] * np.exp(
                    self.exp_step_size * group_loss[group_id]
                )  # update weights based on group loss calculated
    ...
    def batch_update_once(self, dataset: List[GroupTransition], batch_size: int, unique_group_ids: Set[int]) -> float:
        ...
        if self.ipo_grad_type == "noisy_dpo":
            ...
            for group_id in range(self.group_num):
                group_indices = group_id_idx_all[group_id]
                group_loss[group_id] = np.sum(-np.log(sigmoid(log_ratio_diff_all[group_indices]))) + self.adj[
                    group_id
                ] / np.sqrt(
                    self.group_counts[group_id]
                )  # calculate group losses
            ...
            for group_id in range(self.group_num):
                group_indices = group_id_idx_all[group_id]
                group_loss[group_id] = np.sum(np.square(lin_diff[group_indices])) + self.adj[group_id] / np.sqrt(
                    self.group_counts[group_id]
                )
        elif self.ipo_grad_type == "log":
            ...
            log_diff = np.log(
                (
                    cur_policy_act_prob_all[row_indices, pref_act_all]
                    * ref_policy_act_prob_all[row_indices, non_pref_act]
                )
                / (
                    cur_policy_act_prob_all[row_indices, non_pref_act]
                    * ref_policy_act_prob_all[row_indices, pref_act_all]
                )
                + 1e-6
            )
            coef = -2 * (log_diff - 0.5 * (1 / self.reg_coef)) / self.reg_coef
        ...
        neg_cur_data_grad = self.reg_coef * coef * feature_diff_all
        grad = np.sum(-neg_cur_data_grad, axis=0) / len(
            sampled_group_transitions
        )  ############### had self.group_weights[group_id] scaling before
        ...
        if self.l2_reg_rdpo != 0:
            ...
            )  # / len(sampled_group_transitions) # theta-gradient on loss L2 norm λ . ||θ||_F
        self.hist_grad_squared_norm += np.sum(np.square(grad))
        ...
        if self.importance_sampling is False:
            ...
            self.group_weights = self.group_weights * np.exp(
                exp_step_size * group_loss * self.chi
            )  # update weights based on group loss calculated
            self.group_weights = self.group_weights / np.sum(self.group_weights)  # normalize the weights
        ...
        for group_id in range(self.group_num):
            ...
            group_grad_weighted_sum = (
                np.sum(-neg_cur_data_grad[group_indices], axis=0) * worst_avg_tradeoff
            )  # * self.group_weights[group_id]
            if self.importance_sampling == False:  # divide grads by group counts in RDPO
                weighted_group_grad[group_id] = (
                    group_grad_weighted_sum / cur_group_counts[group_id]
                )  # len(sampled_group_transitions)  ############### had self.group_weights[group_id] scaling before
        ...
        if self.l2_reg_rdpo != 0:
            for g in range(self.group_num):
                group_loss[g] += self.l2_reg_rdpo * np.square(
                    np.linalg.norm(self.param)
        ...
        self.param = self.param - step_size * np.sum(
            weighted_group_grad, axis=0
        )  # (self.group_weights @ group_grad) # not just grad but group-grad scaled
    ...
    def batch_update_once_GROUPBATCH(
        self, dataset: List[GroupTransition], batch_size: int, unique_group_ids: Set[int]
    ) -> float:
        ...
        if self.weighted_batches == True:
            ...
            sampled_group_transitions = sample_batch_from_group(
                rand_group
            )  # sample a batch of transitions from this group
        ...
        for transition in sampled_group_transitions:
            ...
            if self.ipo_grad_type == "justdpo":
                ...
                coef = sigmoid(-log_ratio_diff)
                loss += -np.log(sigmoid(log_ratio_diff)) + self.adj[group_id] / np.sqrt(
                    self.group_counts[group_id]
                )  # calculate group losses
            elif self.ipo_grad_type == "linear":
                lin_diff = (feat_pref_act - feat_non_pref_act) @ (self.param) - 0.5 * (1 / self.reg_coef)
                ...
                coef = -2 * (log_diff - 0.5 * (1 / self.reg_coef)) / self.reg_coef
                loss += np.square((log_diff - 0.5 * (1 / self.reg_coef))) + self.adj[group_id] / np.sqrt(
                    self.group_counts[group_id]
                )
            else:
                raise ValueError("value not implemented")
        ...
        if self.importance_sampling == False:
            ...
            self.group_weights[rand_group] = self.group_weights[rand_group] * np.exp(
                self.exp_step_size * loss
            )  # update weights based on group loss calculated
    ...
    def batch_update_closed_form(
        ...
        if self.ipo_grad_type == "justdpo":
            ...
            for group_id in range(self.group_num):
                group_indices = group_id_idx_all[group_id]
                group_loss[group_id] = np.sum(
                    -np.log(sigmoid(log_ratio_diff_all[group_indices]))
                    + self.adj[group_id] / np.sqrt(self.group_counts[group_id])
                )  # calculate group losses
        elif self.ipo_grad_type == "linear":
            lin_diff = feature_diff_all @ self.param.reshape(self.feature_dim, 1) - 0.5 * (1 / self.reg_coef)
            ...
            for group_id in range(self.group_num):
                group_indices = group_id_idx_all[group_id]
                group_loss[group_id] = np.sum(
                    np.square(lin_diff[group_indices]) + self.adj[group_id] / np.sqrt(self.group_counts[group_id])
                )
        elif self.ipo_grad_type == "log":
            row_indices = np.arange(cur_policy_act_prob_all.shape[0])
            log_diff = np.log(
        ...
        for group_id in range(self.group_num):
            group_indices = group_id_idx_all[group_id]
            group_grad[group_id] = np.sum(
                -self.group_weights[group_id] * neg_cur_data_grad[group_indices], axis=0
            ) / len(
                sampled_group_transitions
            )  ############### had self.group_weights[group_id] scaling before
        grad = np.sum(-self.group_weights[group_id] * neg_cur_data_grad, axis=0) / len(
            sampled_group_transitions
        )  ############### had self.group_weights[group_id] scaling before
        group_loss /= cur_group_counts
        ...
        if self.importance_sampling == False:
            ...
            self.group_weights = self.group_weights * np.exp(
                self.exp_step_size * group_loss * self.chi
            )  # update weights based on group loss calculated
            print(self.group_weights)
            self.group_weights = self.group_weights / np.sum(self.group_weights)  # normalize the weights
        self.hist_grad_squared_norm += np.sum(np.square(grad))
    ...
    def evaluate_ipo_loss(self, dataset: List[GroupTransition], policy=None) -> float:
        ...
        if self.ipo_grad_type == "linear":
            ...
            log_diff = np.log(
                (
                    eval_policy_act_prob_all[row_indices, pref_act_all]
                    * ref_policy_act_prob_all[row_indices, non_pref_act_all]
                )
                / (
                    eval_policy_act_prob_all[row_indices, non_pref_act_all]
                    * ref_policy_act_prob_all[row_indices, pref_act_all]
                )
                + 1e-6
            )
    ...
    def evaluate_weighted_ipo_loss(self, dataset: List[GroupTransition], policy=None) -> float:
        ...
        loss = np.sum(
            self.group_weights * np.square(coef) + self.adj[group_id] / np.sqrt(self.group_counts[group_id])
        ) / len(dataset)
        loss = (
            loss * self.group_num
        )  ###for correct comparison as unweighted train loss should multiply 1/num_groups to all
        return loss
    ...
    def evaluate_ipo_grp_loss(self, dataset: List[GroupTransition], policy=None) -> float:
        ...
        for group_id in range(self.group_num):
            group_indices = group_id_idx_all[group_id]
            loss[group_id] = np.sum(
                np.square(coef[group_indices]) + self.adj[group_id] / np.sqrt(self.group_counts[group_id])
            )
    ...
    def evaluate_weighted_loss(self, dataset: List[GroupTransition], policy=None) -> float:
        ...
        for group_id in range(self.group_num):
            group_indices = group_id_idx_all[group_id]
            loss += np.sum(
                -self.group_weights[group_id] * np.log(sigmoid(log_ratio_diff[group_indices]))
                + self.adj[group_id] / np.sqrt(self.group_counts[group_id])
            )
        loss /= len(dataset)
            ...
            if step % self.report_iter == 0:
                ...
                if self.wandb_use:
                    d_wandb = {
                        ...
                        "grad_norm": grad_norm,
                        "live_grad": live_grad,
                        "train_weighted_loss": train_wt_loss,
                        "val_weighted_loss": val_wt_loss,
                        "max_reward_err": max_reward_err,
                        "max_reward_err_index": max_reward_err_index,
                        "max_kl_dist": max_kl_dist,
                        "max_kl_dist_index": max_kl_dist_index,
                        ...
                        "max_val_grp_loss": max_val_grp_loss,
                        "max_val_grp_loss_index": max_val_grp_loss_index,
                        "max_cur_train_grp_loss": max_cur_train_grp_loss,
                        "max_cur_train_grp_loss_index": max_cur_train_grp_loss_index,
                    }
                    # Assuming rew_err is a list
                    for i, err in enumerate(rew_err):
                        key = f"reward_err_{i + 1}"  # Creating dynamic key, e.g., "reward_err_1", "reward_err_2", ...
                    ...
                    for i, grp_ls in enumerate(self.group_loss):
                        key = (
                            f"cur_group_loss_{i + 1}"  # Creating dynamic key, e.g., "reward_err_1", "reward_err_2", ...
                        )
                        d_wandb[key] = grp_ls
                    for i, grp_ls in enumerate(train_grp_loss):
                        key = f"train_group_loss_{i + 1}"  # Creating dynamic key, e.g., "reward_err_1", "reward_err_2", ...
                    ...
                    for i, grp_ls in enumerate(val_grp_loss):
                        key = (
                            f"val_group_loss_{i + 1}"  # Creating dynamic key, e.g., "reward_err_1", "reward_err_2", ...
                        )
                        d_wandb[key] = grp_ls
