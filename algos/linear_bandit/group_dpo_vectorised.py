    ...
    def update_once_nonvectorised(self, dataset: List[GroupTransition]) -> float:
        ...
        for transition in dataset:
            ...
            if self.ipo_grad_type == "justdpo":
                log_ratio_diff = (
                    self.reg_coef * (feat_pref_act - feat_non_pref_act) @ (self.param)
                )  # VECTORISED REDEFINITION instead of log-sum
                coef = sigmoid(-log_ratio_diff)
                group_loss[group_id] += -np.log(
                    sigmoid(log_ratio_diff)
                )  # +self.adj[group_id]/np.sqrt(self.group_counts[group_id]) #calculate group losses
                ...
                group_loss[group_id] += np.square(
                    (log_diff - 0.5 * (1 / self.reg_coef))
    ...
    def update_once(self, dataset: List[GroupTransition]) -> float:
        ...
        if self.ipo_grad_type == "justdpo":
            ...
            for group_id in range(self.group_num):
                group_indices = group_id_idx_all[group_id]
                group_loss[group_id] = np.sum(
                    np.square(lin_diff[group_indices])
                )  # +self.adj[group_id]/np.sqrt(self.group_counts[group_id])
        elif self.ipo_grad_type == "log":
