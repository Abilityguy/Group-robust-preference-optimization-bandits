...
def generate_command(base_config, method_config, feature_type, noise_level, data_size, seed):
    noise_ratio = base_config['noise_ratio_template'].replace('noise_level', str(noise_level))
    weights = base_config['data_ratio_template'].replace('data_size', str(data_size))
    wandb_name = f"{method_config['dpo_type']},{base_config['step_size']},{base_config['reg_coef']},{base_config['exp_step_size']},{noise_ratio},{weights},{seed},IS={method_config['importance_sampling']}"

    timestamp = datetime.now().strftime('%Y_%m_%d_%H_%M_%S')
    log_dir = f"log_{method_config['dpo_type']}/{timestamp}/{feature_type}_{noise_ratio}_{weights}_{seed}"
    ...
    cmd = [
        ...
        f"--flipped_noise={base_config.get('flipped_noise', 'False')}",
        f"--noise_ratio_list={noise_ratio}",
        f"--val_noise_ratio_list={base_config['val_noise_ratio_list']}",
...
def main(args):
    ...
    with open(args.commands_file, 'w') as f:
        for method_config in method_configs:
            for feature_type in base_config['feature_types']:
                for noise_level in base_config['noise_levels']:
                    for data_size in base_config['data_sizes']:
                        for seed in base_config['seeds']:
                            cmd = generate_command(base_config=base_config, method_config=method_config, feature_type=feature_type, noise_level=noise_level, data_size=data_size, seed=seed)
                            f.write(f"{cmd}\n")
